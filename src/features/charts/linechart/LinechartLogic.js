...
export const linechart = (selection, props) => {
    ...
    const background = selection.selectAll('.backgroundLineChart').data([null]);
    background.enter().append('rect')
            .attr('class', 'backgroundLineChart')
        .merge(background)
            .attr("height", svgHeight)
            .attr("width", svgWidth)
            .attr("fill", `rgba(${svgBg.r}, ${svgBg.g}, ${svgBg.b}, ${svgBg.a})`);

    const g = selection.selectAll('.containLineChart').data([null]);
    const gEnter = g.enter().append('g')
        .attr('class', 'containLineChart');
